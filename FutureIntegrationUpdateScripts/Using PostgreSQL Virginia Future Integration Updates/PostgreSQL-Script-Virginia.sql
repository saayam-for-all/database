...
-- Enforce that phone_country_code is greater than 0 (Issue 4)
ALTER TABLE country
  ADD CONSTRAINT chk_country_phone_code_positive
  CHECK (phone_country_code > 0);
...
-- Enforce that city names are unique within a state (Issue 4)
ALTER TABLE city
  ADD CONSTRAINT uq_city_state_name
...
CREATE TABLE IF NOT EXISTS users (
    ...
    -- Removed full_name to avoid redundancy (Issue 2)
    ...
    -- Initially keep city_name for data migration purposes (Issue 3)
...
-- Enforce unique primary_email_address for data integrity (Issue 4)
ALTER TABLE users
  ADD CONSTRAINT users_primary_email_unique
  UNIQUE (primary_email_address);
...
CREATE TABLE IF NOT EXISTS volunteer_details (
    ...
    user_id VARCHAR(255) NOT NULL,
    -- Enforce terms_and_conditions to be known (not NULL) (Issue 4)
    terms_and_conditions BOOL NOT NULL,
...
-- ========================================
-- Table: user_volunteer_skills (Unstructured volunteer skills)
-- ========================================
...
CREATE TABLE IF NOT EXISTS request (
    ...
    request_user_id VARCHAR(255) NOT NULL,  -- User who created/owns the request
    request_status_id INT NOT NULL,
    request_priority_id INT NOT NULL,
    request_type_id INT NOT NULL,
    request_category_id INT NOT NULL,
    request_for_id INT NOT NULL,
    city_name VARCHAR(255) NOT NULL,  -- Retained for now for data migration
    zip_code VARCHAR(255) NOT NULL,
    request_desc VARCHAR(255) NOT NULL,
    audio_req_desc VARCHAR(255) NULL,
    request_for VARCHAR(255) NOT NULL,
    submission_date TIMESTAMP NOT NULL,  -- Enforced NOT NULL (Issue 4)
    serviced_date TIMESTAMP NOT NULL,    -- Enforced NOT NULL (Issue 4)
    last_update_date TIMESTAMP NOT NULL, -- Enforced NOT NULL (Issue 4)
    lead_volunteer_user_id VARCHAR(255),  -- Converted to VARCHAR(255) to reference users (Issue 1)
    UNIQUE (request_id),
...
-- Add foreign key for lead_volunteer_user_id (Issue 1)
ALTER TABLE request
  ADD CONSTRAINT fk_request_lead_volunteer
  FOREIGN KEY (lead_volunteer_user_id)
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    status status_type NOT NULL,  -- Enforce NOT NULL (Issue 4)
...
-- Enforce that the same skill_desc does not repeat within a request category (Issue 4)
ALTER TABLE skill_list
  ADD CONSTRAINT uq_skill_list_desc
  UNIQUE (request_category_id, skill_desc);
...
-- Indexes for volunteer_rating table
...
-- Enforce unique day_of_week per user (Issue 4)
ALTER TABLE user_availability
  ADD CONSTRAINT uq_user_availability_day
  UNIQUE (user_id, day_of_week);

-- Enforce that start_time is less than end_time (Issue 4)
ALTER TABLE user_availability
...
-- Sequence for generating user IDs
...
-- Function to generate formatted user IDs
...
-- Function to generate formatted request IDs
...
-- ========================================
-- Data Migration for City Reference (Issue 3)
-- ========================================
-- Step 1: Add new column city_id to users
ALTER TABLE users
  ADD COLUMN IF NOT EXISTS city_id INT;

-- Step 2: Migrate existing data from city_name to city_id.
-- This assumes that users.city_name matches exactly with city.city_name.
UPDATE users u
  SET city_id = c.city_id
  FROM city c
  WHERE u.city_name = c.city_name;

-- Step 3: Remove the redundant city_name column from users
ALTER TABLE users
  DROP COLUMN city_name;

-- Step 4: Add a foreign key constraint on the new city_id column
ALTER TABLE users
