...
3. Run the search scripts from `../` in order.
...
```bash
psql -d saayam_search_clone_test -f tests/Search/runners/run_virginia_search_validation.sql
```
...
psql -d saayam_search_clone_test -f tests/Search/test_clones/virginia_search_instance_clone.sql
psql -d saayam_search_clone_test -f tests/Search/runners/run_virginia_search_migrations.sql
psql -d saayam_search_clone_test -v ON_ERROR_STOP=1 -f tests/Search/runners/run_virginia_search_validation.sql
```
...
psql -d saayam_ireland_search_clone_test -f tests/Search/test_clones/ireland_search_instance_clone.sql
psql -d saayam_ireland_search_clone_test -f tests/Search/runners/run_ireland_clone_validation.sql
```
...
The same production scripts under `ddl/Search/` are used for Ireland.
The Ireland migration runner sets `search_path` to `proposed_saayam`.

```bash
createdb saayam_ireland_full_search_test
psql -d saayam_ireland_full_search_test -f tests/Search/test_clones/ireland_search_instance_clone.sql
psql -d saayam_ireland_full_search_test -f tests/Search/runners/run_ireland_search_migrations.sql
psql -d saayam_ireland_full_search_test -v ON_ERROR_STOP=1 -f tests/Search/runners/run_ireland_search_validation.sql
```
...
```bash
psql -d saayam_search_clone_test -f tests/Search/virginia_validation/01_search_smoke_test.sql
```
...
```bash
psql -d saayam_search_clone_test -f tests/Search/virginia_validation/02_search_index_check.sql
```
...
```bash
psql -d saayam_search_clone_test -f tests/Search/virginia_validation/03_search_function_check.sql
```
...
```bash
psql -d saayam_search_clone_test -f tests/Search/virginia_validation/04_instance_clone_check.sql
```
