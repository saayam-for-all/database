-- This table is only populated when req_for is 1/OTHER  in the 'requests' table.
CREATE TABLE IF NOT EXISTS virginia_dev_saayam_rdbms.request_guest_details (
    -- The primary key is the foreign key to the requests table (one-to-one relationship)
    ...
    FOREIGN KEY (req_id) REFERENCES virginia_dev_saayam_rdbms.requests (req_id) ON DELETE CASCADE
);
