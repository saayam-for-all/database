...
CREATE TABLE IF NOT EXISTS virginia_dev_saayam_rdbms.organizations (
    ...
    -- TRUE  = collaborator (onboarded by marketing team)
    -- FALSE = individually registered on the platform
    is_collaborative    BOOLEAN,

    -- TRUE  = paid / for-profit contributing org
    -- FALSE = unpaid / non-contributing org
...
-- Partial index: quickly locate all collaborative partner orgs
CREATE INDEX idx_org_collaborative
    ON virginia_dev_saayam_rdbms.organizations(is_collaborative)
    WHERE is_collaborative = TRUE;

-- Partial index: quickly locate all for-profit contributing orgs
CREATE INDEX idx_org_contributing
    ON virginia_dev_saayam_rdbms.organizations(is_contributing)
    WHERE is_contributing = TRUE;

-- ── Auto-ID trigger function ───
