...
AS $$
DECLARE
    k BIGINT;-- PostgreSQL Script adapted for Ireland Region
-- User ID Generation, New columns in User Table
-- Renaming tables with their attributes: region for better understanding
-- Changes in Table: volunteer_organizations
-- Added WHEN clause to migrate data with existing IDs
...
-- =============================================
-- Stockholm Users Table
-- Handles EU users during Ireland DR and
-- non-EU users during Virginia DR
-- EU DR Range:     20 billion to 40 billion
-- Non-EU DR Range: 40 billion to 60 billion
-- =============================================

CREATE EXTENSION IF NOT EXISTS postgis;

-- Keep updated_at fresh
CREATE OR REPLACE FUNCTION stockholm_dev_saayam_rdbms.set_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.last_updated_at := NOW();
  RETURN NEW;
...
CREATE OR REPLACE FUNCTION stockholm_dev_saayam_rdbms.set_promo_wizard_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  IF (NEW.promotion_wizard_last_updated_at IS DISTINCT FROM OLD.promotion_wizard_last_updated_at)
  THEN NEW.promotion_wizard_last_updated_at = (NOW() AT TIME ZONE 'UTC');
...
CREATE TABLE IF NOT EXISTS stockholm_dev_saayam_rdbms.users (
    user_id VARCHAR(25) PRIMARY KEY,
    state_id VARCHAR(30) NULL,
    country_id INT NULL,
    user_status_id INT NULL,
    ...
    dob DATE,
    is_eu BOOLEAN DEFAULT TRUE,
    FOREIGN KEY (country_id) REFERENCES stockholm_dev_saayam_rdbms.country (country_id) ON DELETE SET NULL,
    FOREIGN KEY (state_id) REFERENCES stockholm_dev_saayam_rdbms.state (state_id) ON DELETE SET NULL,
    FOREIGN KEY (user_status_id) REFERENCES stockholm_dev_saayam_rdbms.user_status (user_status_id),
    FOREIGN KEY (language_1) REFERENCES stockholm_dev_saayam_rdbms.supporting_languages(language_id) ON DELETE SET NULL,
    FOREIGN KEY (language_2) REFERENCES stockholm_dev_saayam_rdbms.supporting_languages(language_id) ON DELETE SET NULL,
    FOREIGN KEY (language_3) REFERENCES stockholm_dev_saayam_rdbms.supporting_languages(language_id) ON DELETE SET NULL
);

CREATE TRIGGER trg_users_updated_at
BEFORE UPDATE ON stockholm_dev_saayam_rdbms.users
FOR EACH ROW
EXECUTE FUNCTION stockholm_dev_saayam_rdbms.set_updated_at();

CREATE TRIGGER trg_users_promo_wizard_updated_at
BEFORE UPDATE ON stockholm_dev_saayam_rdbms.users
FOR EACH ROW
EXECUTE FUNCTION stockholm_dev_saayam_rdbms.set_promo_wizard_updated_at();

-- EU DR: 20 billion to 40 billion
CREATE SEQUENCE stockholm_dev_saayam_rdbms.user_id_seq_eu_dr
    START WITH 20000000001
    INCREMENT BY 1
    MINVALUE 20000000001
    MAXVALUE 40000000000
...
-- Non-EU DR: 40 billion to 60 billion
CREATE SEQUENCE stockholm_dev_saayam_rdbms.user_id_seq_non_eu_dr
    START WITH 40000000001
    INCREMENT BY 1
...
BEGIN
    IF NEW.is_eu = TRUE THEN
        seq_id     := nextval('stockholm_dev_saayam_rdbms.user_id_seq_eu_dr');
        sid_prefix := 'SID-EU-';
    ELSE
        seq_id     := nextval('stockholm_dev_saayam_rdbms.user_id_seq_non_eu_dr');
        sid_prefix := 'SID-001-';
    END IF;
...
CREATE TRIGGER before_insert_users
BEFORE INSERT ON stockholm_dev_saayam_rdbms.users
FOR EACH ROW
EXECUTE FUNCTION stockholm_dev_saayam_rdbms.generate_sid();
BEGIN
    -- Serialize concurrent inserts for the same user
    k := hashtextextended(NEW.user_id, 0);
    PERFORM pg_advisory_xact_lock(k);
