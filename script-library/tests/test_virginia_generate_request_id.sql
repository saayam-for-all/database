...
DROP TABLE IF EXISTS _va_req_id_seq_snapshot;
CREATE TEMP TABLE _va_req_id_seq_snapshot AS
SELECT last_value, is_called
FROM virginia_dev_saayam_rdbms.request_id_seq;
...
-- T2  Exhaustion hard wall at MAXVALUE 999,999,999,999 ---------------------
DO $$
DECLARE
    last_id TEXT;
    uid TEXT;
BEGIN
    ...
    IF last_id <> 'REQ-099-999-999-9999' THEN
        RAISE NOTICE 'T2 FAIL  last mint expected REQ-099-999-999-9999 got %', last_id;
        RETURN;
    END IF;
...
SELECT setval(
    'virginia_dev_saayam_rdbms.request_id_seq',
    last_value,
    is_called
)
FROM _va_req_id_seq_snapshot;
